...
function deleteAnalysisById(id, email) {
  const normEmail = email ? email.toLowerCase() : null;
  try {
    if (fs.existsSync(HISTORY_FILE)) {
      let history = JSON.parse(fs.readFileSync(HISTORY_FILE, 'utf-8'));
    ...
    console.error('Failed to delete history entry', id, err);
  }
  return false;
}

module.exports = { saveAnalysis, getHistory, getUsageCount, clearHistory, getAnalysisById, deleteAnalysisById };
