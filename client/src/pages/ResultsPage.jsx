...
export default function ResultsPage({ isProUser }) {
  ...
  const matchedSkills = (atsResult.matched_skills || []).map(s => (s && typeof s === 'object') ? s.skill : s).filter(Boolean);
  const missingSkills = (atsResult.missing_skills || []).map(s => (s && typeof s === 'object') ? s.skill : s).filter(Boolean);
