...
function PricingPage({ billingYearly, setBillingYearly, showToast, setPlan, plan }) {
  ...
  const plans = [
    ...
    { name: 'Pro Member', amt: proAmt, bill: proBill, features: ['Unlimited analyses', 'Priority processing', 'Enhanced written insights', 'Full history tracking', 'Priority support'], cta: plan === 'pro' ? 'Revert to Free' : 'Join Waitlist', href: '#', featured: true },
                ...
                <button 
                  onClick={() => {
                    if (p.href !== '#') {
                      ...
                      if (plan === 'pro') {
                        setPlan('free');
                        showToast('Plan reverted to Free');
                      } else {
                        setPlan('pro');
                        showToast('Developer Mode: Pro activated!');
                      }
                    }
                  }}
...
function LoadingScreen() {
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center bg-slate-50/50 font-inter">
      <div className="w-10 h-10 border-4 border-slate-100 border-t-blue-600 rounded-full animate-spin mb-4" />
      <div className="text-slate-400 font-black uppercase tracking-widest text-[10px]">Initializing Session...</div>
    </div>
  );
}
