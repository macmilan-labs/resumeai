  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div>
           <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={fillSample} className="text-[10px] font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg">Use Sample</button>
          </div>
          <textarea 
            value={jd} 
            onChange={e => setJd(e.target.value)} 
            placeholder="Paste the target job description here..."
            className="w-full min-h-[300px] p-8 text-sm text-slate-700 bg-slate-50/30 border border-slate-200 rounded-[2.5rem] font-medium focus:bg-white focus:border-blue-500 transition-all outline-none leading-relaxed"
          />
        </div>
        <div>
      ...
      <button 
        ...
        disabled={loading} 
        className="w-full px-10 py-5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white font-black text-xs uppercase tracking-[0.2em] rounded-[2.5rem] shadow-xl transition-all flex items-center justify-center gap-4 group active:scale-95"
      >
        {loading ? (
          <>
